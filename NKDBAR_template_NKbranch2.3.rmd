...
# unzipped version
E_flats <- read_csv("Data Sample (Renamed Units)/iUnit_Eliot_R_export_sample_data_201610_201612/iUnit_Eliot_R_export_sample_data_201610.csv")
...
super.file.list <- list.files(path = "Data Sample (Renamed Units)", pattern = '^[^.]+$')

# Compile a list of relevant files in the directory
file.list <- list.files(path = "Data Sample (Renamed Units)/iUnit_Eliot_R_export_sample_data_201610_201612", pattern='*iUnit_Eliot_R_export_sample_data')
...
{
  data <- read_csv(
    paste("Data Sample (Renamed Units)/iUnit_Eliot_R_export_sample_data_201610_201612/", file.list[i], sep = "")
...
Understand the DHW heater characteristics from the submetered unit 21. There is a strong correlation between the total kW and water heater loads fir unit 21. 
```{r}
library(corrplot)
corrMat<-cor(Submeters[4:11])
corrplot(corrMat,method='ellipse')
# since there is a strong correlation between total kW and water heater, plot water heater to see its trend over time
attach(Submeters)
plot(UnitAnon21.Water.Heater.kW)
# check how unit21.total.kw and unit21.kw variables differ 
mean(Submeters$UnitAnon21.kW)>mean(Submeters$UnitAnon21.Total.kW)
```
Aggregate the data at different time scales. 
```{r}
# Create an aggregated data frame of the maximum Submetered demand for every fifteen minute interval
Submeters.15.mins <- Submeters %>%
  group_by(Time.Local.Text = cut(Time.Local.Text, breaks = "15 min")) %>%
  summarise_all(funs(max))
# Create an aggregated data frame of the maximum Submetered demand for every one hour interval
Submeters.1.hr <- Submeters %>%
  group_by(Time.Local.Text = cut(Time.Local.Text, breaks = "1 h")) %>%
  summarise_all(funs(max))
# Create an aggregated data frame of the maximum Submetered demand for every twenty-four hours interval
Submeters.24.hr <- Submeters %>%
  group_by(Time.Local.Text = cut(Time.Local.Text, breaks = "24 h")) %>%
  summarise_all(funs(max))
```
Now, we can plot the water heater to see its trends. 
```{r}
# trying to plot the data with 15 minutes time interval 
attach(Submeters.15.mins)
plot(UnitAnon21.Water.Heater.kW,main="Trends of Water heater consumption on 15 minutes time interval data")
# trying to plot the data with 1 hour time interval 
attach(Submeters.1.hr)
plot(UnitAnon21.Water.Heater.kW,main="Trends of Water heater consumption on one hour time interval data")
# trying to plot the data with 24 hours time interval 
attach(Submeters.24.hr)
plot(UnitAnon21.Water.Heater.kW,main="Trends of Water heater consumption on 24 hours time interval data")
```
```{r}
# trying to plot the total kW of unit 21 data with 15 minutes time interval 
attach(Submeters.15.mins)
plot(UnitAnon21.Total.kW,main="Trends of total kW on 15 minutes time interval data",type="l")
# trying to plot the data with 1 hour time interval 
attach(Submeters.1.hr)
plot(UnitAnon21.Total.kW,col="red",main="Trends of unit 21 events based on one hour time interval data",type="l")
points(UnitAnon21.Water.Heater.kW, pch = 20,col="blue")
points(UnitAnon21.Washer.kW,pch = 20,col="yellow")
points(UnitAnon21.Dryer.kW,pch = 20,col="pink")
points(UnitAnon21.Range.kW,pch = 20,col="green")
points(UnitAnon21.PTAC.kW,pch = 20,col="black")
points(UnitAnon21.Living.Plug.kW,pch = 20,color="grey")
legend(x="topright",pch=c(20,20,20),col=c("red","blue","yellow","pink","green","black","grey"),legend=c("Total.kW", "Washer.kW", "Dryer.kW", "Range.kW", "PTAC.kW", "Living.Plug.kW"))
# trying to plot the data with 24 hours time interval 
attach(Submeters.24.hr)
plot(UnitAnon21.Total.kW,pch = 20,col="red",type="l",main="Trends of unit 21 events based on 24 hours time interval data")
points(UnitAnon21.Water.Heater.kW, pch = 20,col="blue")
points(UnitAnon21.Washer.kW,pch = 20,col="yellow")
points(UnitAnon21.Dryer.kW,pch = 20,col="pink")
points(UnitAnon21.Range.kW,pch = 20,col="green")
...
legend(x="topright",pch=c(20,20,20),col=c("red","blue","yellow","pink","green","black","grey"),legend=c("Total.kW", "Washer.kW", "Dryer.kW", "Range.kW", "PTAC.kW", "Living.Plug.kW"))
```
Yield a minute-by-minute time series of binary DHW activity.
```{r}
# create a new vector to yield a minute-by-minute time series of binary DHW activity.(0="OFF',1="ON")
Submeters$binary.DHW <- ifelse(Submeters$UnitAnon21.Water.Heater.kW >= 0 , 1, 0)
# create a new vector to yield a 15 minutes time series of binary DHW activity.
Submeters.15.mins$binary.DHW <- ifelse(Submeters.15.mins$UnitAnon21.Water.Heater.kW >= 0 , 1, 0)
# create a new vector to yield a 1 hour time series of binary DHW activity.
Submeters.1.hr$binary.DHW <- ifelse(Submeters.1.hr$UnitAnon21.Water.Heater.kW >= 0 , 1, 0)
# create a new vector to yield a 24 hours time series of binary DHW activity.
Submeters.24.hr$binary.DHW <- ifelse(Submeters.24.hr$UnitAnon21.Water.Heater.kW >= 0 , 1, 0)
```
Now, understand what's happening when the water heater is either on or off 
```{r}
# extracting the submetered data when the DHW is ON (=1)
Submeters.ON.DHW <- Submeters %>%
  filter(binary.DHW==1) #7189 observations
# extracting the submeters data when the DHW is OFF (=0)
Submeters.OFF.DHW <- Submeters %>%
  filter(binary.DHW == 0) #125,351 obs from the minute by minute data 
```

```{r}
# develop box plots for the submetered events of unit 21 when the DHW is on 
boxplot(Submeters.ON.DHW$UnitAnon21.Total.kW,
        Submeters.ON.DHW$UnitAnon21.PTAC.kW,
        Submeters.ON.DHW$UnitAnon21.Water.Heater.kW,
        Submeters.ON.DHW$UnitAnon21.Washer.kW,
        Submeters.ON.DHW$UnitAnon21.Dryer.kW,
        Submeters.ON.DHW$UnitAnon21.Range.kW, 
        Submeters.ON.DHW$UnitAnon21.Living.Plug.kW)

# see the trends of AC consumption when the DHW is on 
plot(Submeters.ON.DHW$UnitAnon21.PTAC.kW,col="blue",main="Trends of AC consumption when DHW is ON")

# see the trends of AC consumption when the DHW is off
plot(Submeters.OFF.DHW$UnitAnon21.PTAC.kW,col="red",main="Trends of AC consumption when DHW is OFF")
# people will likely turn the AC on when they're not taking showers and will turn it off when they are taking showers. Thus, it was expected to see those trends of AC consumption. 
```
Now, we can develop rules and models based on unit 21 events, to be used for the rest of 39 units. 
```{r}
```

use different method to fit a model and then predict the events for the rest of 39 units 
